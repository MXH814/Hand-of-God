...
- 第 3 关 `Create & Erase`（创造和消除）提供绘制创造、物体摆放、交叉消除锁块和侧向滑桥机关。
...
校准完成后，游戏界面左侧会显示 `Level Select` 选关栏。玩家可以用食指悬停直接进入 `Level 0: Tutorial`、`Level 1: Moving Path`、`Level 2: Portals` 或 `Level 3: Create & Erase`（创造和消除）；在关卡中选择当前关卡会重新开始该关。
...
- 用双手食指交叉选中侧向滑桥的锁块，锁块变为高亮材质并显示 2 秒进度条。
...
主菜单和左侧 `Level Select` 可直接进入 `Level 3: Create & Erase`（创造和消除）；Level 2 通关后的 `PASS` 界面会显示 `Next: Level 3`。Level 3 通关后只显示 `Restart` 和 `Tutorial`。
...
- 光束使用透明粒子贴图材质绘制为“亮芯 + 柔光外晕”两层，不再表现为实体杆。
...
- `displayLandmarks` 直接使用原始 MediaPipe 点；Unity 绘制骨架时直接跟随当前点，仅对单帧异常大跳变做保护，真实弯曲动作会立即同步到骨架显示；UI 和机关判定不直接依赖这套显示点。
...
实际运行时会根据姿态动态调整平滑强度：捏合拖动物体时提高 pinch center 响应速度；只伸出食指且其他手指弯曲时，对低幅抖动保留过滤，但真实移动会用更高更新系数快速跟随，减少 UI 悬停抖动的同时避免指尖和指节滞后。骨架显示优先保持实时手型，UI 悬停稳定性主要由 cursor EMA 承担。
...
- 第一机关完成后，交叉食指选中滑桥锁块并保持 2 秒可释放侧向滑桥。

...
- 可用 `--camera-fps` 调整摄像头目标帧率，用 `--capture-width` / `--capture-height` 调整 MediaPipe 处理分辨率，用 `--video-width` / `--video-height` / `--video-fps` 调整 Unity 内嵌摄像头画面，也可用 `--detection-confidence` / `--tracking-confidence` 调整 MediaPipe 置信度阈值；桥接每 5 秒向 `gesture-bridge-runtime.log` 输出一次 FPS、处理耗时、采集分辨率、视频帧率、模型复杂度和 tracking 模式。

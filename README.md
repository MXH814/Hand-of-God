...
- 技术：新增配置化关卡数据，把起点、终点、掉落边界、物理平台、视觉危险区和装饰物从 `ShapeScene` 的硬编码中拆出；`ShapeScene` 根据关卡配置同时生成 Three.js mesh 和 cannon-es body。渲染层启用柔和阴影，调试几何体托盘改为弱化的右上角浮层，避免抢占主地图视觉中心。

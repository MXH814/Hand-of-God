...
- 算法/规则：每只手取食指根部 `5` 到食指尖 `8` 作为食指线段；两只手食指均伸展、两条线段在 `ar-stage` 屏幕坐标内相交且夹角大于阈值时，使用交叉点做 Three.js 命中测试；raycaster 返回该位置最前方对象，删除命中的任意几何体，并设置 900ms 冷却避免一次交叉连续删除多个对象。

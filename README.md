...
- 只有亚像素级极小位移才进一步降低 `alpha`，过滤握拳、弯曲手指和单指指向时的微小跳动，避免真实关节动作被拖慢。
...
实际运行时会根据姿态动态调整平滑强度：捏合拖动物体时提高 pinch center 响应速度；只伸出食指且其他手指弯曲时，对低幅抖动保留过滤，但真实移动会用更高更新系数快速跟随，减少 UI 悬停抖动的同时避免指尖和指节滞后。

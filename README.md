...
| 8/9 Draw a new object | 左右食指指尖重合 1 秒确认开始；开始后只记录右手食指轨迹，左手移动不会结束绘制；右手握拳 1 秒取消；绘制后左右食指再次重合 1 秒确认结束 | 轨迹闭合后识别圆形、矩形或三角形；圆形生成近似半径的球体；矩形生成近似长宽并跟随绘制方向的立方体；三角形生成近似底面和高度的三棱锥；无法识别则提示并取消 |

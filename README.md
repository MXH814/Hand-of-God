...
默认 `displayLandmarks` 由 `ResponsiveFingerDisplayHand` 生成：Unity 绘制骨架时对这套显示点直绘，不再做单个指节限速；Python 仅对“指尖动得很快、中间指节明显没跟上”的帧做有限幅度的 PIP / DIP 补偿，并用当前帧 MCP、PIP、DIP、tip 骨段长度做多轮软约束，避免第一指节慢半拍造成的“折断感”或补偿后骨架明显变形。当非拇指手指整体仍符合伸直几何特征、但 PIP / DIP 因双手手指交叉或遮挡被识别到 MCP-tip 直线外侧时，会把中间指节轻柔拉回直线，减少伸直手指在交叉处抖动或突然弯曲；真实卷曲手势因 MCP-tip 直线距离明显缩短，不会触发这层拉直。随后显示层使用基于掌宽的 deadband：目标点在几像素量级内抖动时保持上一显示位置，刚超过死区的小动作低增益跟随，大动作按运动幅度快速跟随。

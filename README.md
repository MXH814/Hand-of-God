...
| 气流指向手势 | 第 2 关设置气流方向 | 单手食指和中指并拢、拇指伸出、无名指和小指收起；Unity 用食指相对手腕的 X 方向判断左风或右风 |
...
| 分段 | 名称 | 机关 | 使用手势 | 目标 |
| --- | --- | --- | --- | --- |
| Segment 1 | 钥匙激活区 | Portal Key、符文、Portal A/B | 单手捏合拖动 | 捏住钥匙并放到符文上，激活传送门 |
| Segment 2 | 传送区 | Portal A、Portal B、第一道挡门 | 符文触发 | 激活后小球立即从 Portal A 传送到 Portal B，并打开第一道挡门 |
| Segment 3 | 气流控制区 | Air Belt、第二道挡门 | 气流指向手势 | 食指中指并拢、拇指伸出，向右指向以开启右风 |
| Segment 4 | 终点区 | 气流带、终点祭坛 | 物理推动 | 气流推动小球进入终点，显示 `PASS` |

第 2 关的交互细节：

- 钥匙靠近手势点时会显示 hover 高亮；玩家捏合靠近钥匙时会进入抓取。
- 钥匙有较大的磁吸抓取半径：玩家只要在钥匙附近捏合，钥匙会先吸附到捏合点，再进入拖动状态。
- 放开钥匙且钥匙位于符文附近时，符文、Portal A 和 Portal B 都变为激活高亮。
- 传送门激活后，小球会立即移动到 Portal B 附近，清空速度并唤醒 Rigidbody。
- 气流手势由 `IndexMiddleTogether + thumbExtended + !ringExtended + !pinkyExtended` 组成。
- Unity 根据食指尖相对手腕的水平偏移判断风向：向右为通关方向，向左会显示提示要求改为向右。
- 气流带使用 `AirBeltTrigger`，在小球停留于触发区时持续施加水平方向加速度；如果速度过低，会给一个最低速度补偿，保证机关反馈明显。
- 风向设置为右后，第二道挡门打开，HUD 显示 `Wind direction: RIGHT - Ball will move to goal!`。
...
- 如果小球在该方向上的速度低于 `boostThreshold`，直接补到 `boostSpeed`，避免气流反馈太弱。
...
- 小球从 Portal A 传送到 Portal B，并打开第一道挡门。
- 气流指向手势可被识别：食指中指并拢、拇指伸出、无名指和小指收起。

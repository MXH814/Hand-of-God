...
class SmoothedPoint:
    ...
    def update(self, x, y, z, index=-1):
        ...
        if is_palm_anchor:
            alpha = 0.18 + min(speed * 0.026, 0.24)
            max_alpha = 0.48
            min_alpha = 0.075
        else:
            alpha = 0.46 + min(speed * 0.075, 0.34)
            max_alpha = 0.82
            min_alpha = 0.16

        if is_fingertip:
            alpha *= 0.96
        alpha -= max(edge - 0.72, 0.0) * 0.05
        if displacement < 0.0008:
            alpha *= 0.50
        elif displacement < 0.0020:
            alpha *= 0.78
        ...
        while cap.isOpened():
            ...
            if result.multi_hand_landmarks:
                ...
                for index, hand in enumerate(result.multi_hand_landmarks[:2]):
                    ...
                    pinch_alpha = 0.55 if latched else 0.35
                    ...
                    if index_only:
                        index_alpha = 0.18 if index_motion < 0.018 else 0.38
                    else:
                        index_alpha = 0.45
                    smoothed = (
